...
export async function listConsultations(workspaceId:string) { const q=await (await getDb()).prepare(`${select} WHERE workspace_id=? ORDER BY created_at DESC LIMIT 500`).bind(workspaceId).all<Row>(); return q.results.map(map); }
