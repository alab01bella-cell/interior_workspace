...
async function resourceIds(workspaceId:string,workspaceName:string,token:string,rootId:string){
  ...
  await initializeConsultationSpreadsheet(token,spreadsheet,summaryHeaders,rawHeaders); return {parent,spreadsheet};
}
...
export async function syncConsultationStatus(workspaceId:string,id:string,workspaceName:string){const record=await findConsultation(workspaceId,id);if(!record)return;try{const connection=await findDriveConnection(workspaceId);if(!connection?.driveRootFolderId)throw new Error("google_connection_unavailable");const token=await getGoogleAccessToken(connection);const resources=await resourceIds(workspaceId,workspaceName,token,connection.driveRootFolderId);await updateStatusCell(token,resources.spreadsheet,id,STATUS_FROM_DB[record.status]);await updateSync(id,{status:record.driveFolderId?"SYNCED":"PARTIAL",sheetSynced:true});}catch(error){const code=error instanceof Error?error.message:"google_sync_failed";await updateSync(id,{status:code==="google_permission_required"?"PERMISSION_REQUIRED":"PARTIAL",error:code});}}
