...
const driveFileError=(await searchParams).driveFile;
return <AppShell identity={toWorkspaceIdentity(context)}>
<main className="customer-detail-page">{driveFileError&&<p className="file-missing-notice">{driveFileError==="reauth"?<>Google Drive 연결을 다시 확인해주세요. <Link href="/settings/integrations">다시 연결</Link></>:driveFileError==="config"?"Google Drive 설정을 확인해주세요.":driveFileError==="permission"?"Google Drive 파일 접근 권한이 없습니다.":driveFileError==="not_found"?"상담기록 파일을 찾을 수 없습니다.":"파일을 불러오지 못했습니다. 잠시 후 다시 시도해주세요."}</p>}<header className="customer-detail-header">
<div>
<p>CONSULTATION</p>
<div>
