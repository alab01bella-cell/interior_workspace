...
const statuses: Array<ConsultationStatus|"불성사"> = ["접수", "예약", "완료", "계약", "불성사"];

export default async function ConsultationsRoute({ searchParams }: { searchParams: Promise<{ status?: string }> }) {
  const context = await requireWorkspace();
  const { status } = await searchParams;
  const initialStatus = statuses.includes(status as ConsultationStatus|"불성사") ? status as ConsultationStatus|"불성사" : "전체";
