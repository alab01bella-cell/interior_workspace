...
export default async function ProfileSettingsRoute(){const context=await requireWorkspace();const connection=await findPublicDriveConnection(context.workspace.id);return <AppShell identity={toWorkspaceIdentity(context)}><ProfileSettingsPage name={getServiceDisplayName(context.user)} email={context.user.email} jobTitle={context.user.jobTitle??""} initialImageUrl={context.user.profileImageUrl} hasCustomImage={Boolean(context.user.customProfileDriveFileId)} driveConnected={connection?.connectionStatus==="CONNECTED"&&Boolean(connection.driveRootFolderId)}/></AppShell>}
