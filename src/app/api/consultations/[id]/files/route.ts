...
export async function GET(request:Request,{params}:{params:Promise<{id:string}>}){
  ...
  let reconciliation:"not_requested"|"ok"|"drive_connection_unavailable"|"drive_folder_unavailable"|"reauth_required"|"config_error"|"permission_error"|"temporary_error"="not_requested",discovered=0;
  if(new URL(request.url).searchParams.get("reconcile")==="1"){
    if(!consultation.driveFolderId)reconciliation="drive_folder_unavailable";
    else {const connection=await findDriveConnection(context.workspace.id);if(!connection)reconciliation="drive_connection_unavailable";else try{const result=await reconcileConsultationDriveFiles({workspaceId:context.workspace.id,consultationId:id,consultationFolderId:consultation.driveFolderId,accessToken:await getGoogleAccessToken(connection)});reconciliation="ok";discovered=result.discovered;}catch(error){const kind=driveErrorKind(error);reconciliation=kind==="REAUTH_REQUIRED"?"reauth_required":kind==="CONFIG_ERROR"?"config_error":kind==="PERMISSION_ERROR"?"permission_error":"temporary_error";}}
  }
...
export async function POST(request:Request,{params}:{params:Promise<{id:string}>}){
  ...
  const connection=await findDriveConnection(context.workspace.id);if(!connection)return NextResponse.json({error:"reauth_required"},{status:409});
