...
export async function PATCH(request:NextRequest,{params}:{params:Promise<{id:string}>}){
  ...
  if(!body||(body.outcome!=="CONTRACTED"&&body.outcome!=="LOST")||!(body.lostReason===null||typeof body.lostReason==="string"&&reasons.has(body.lostReason as LostReason))||!(body.lostReasonNote===null||typeof body.lostReasonNote==="string")||String(body.lostReasonNote??"").length>2000||typeof body.idempotencyKey!=="string"||body.idempotencyKey.length<8||body.idempotencyKey.length>100)return NextResponse.json({error:"invalid_request"},{status:400});
