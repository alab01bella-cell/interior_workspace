...
function Field({ children, label, hint }: { children: ReactNode; label: string; hint?: string }) {
  return <div className={styles.field}><label className={styles.fieldLabel}>{label}</label>{children}{hint && <p className={styles.hint}>{hint}</p>}</div>;
}

function TextField({ form, label, name, placeholder = "", textarea = false, type = "text", min, inputMode, onChange }: {
  form: ChecklistFormState; label: string; name: string; placeholder?: string; textarea?: boolean; type?: string; min?: string;
  inputMode?: "numeric"; onChange: (name: string, value: string) => void;
}) {
  ...
  return <Field label={label}>{textarea ? <textarea name={name} onChange={handleChange} placeholder={placeholder} value={value} /> : <input inputMode={inputMode} min={min} name={name} onChange={handleChange} placeholder={placeholder} type={type} value={value} />}</Field>;
}

function ChoiceField({ form, label, name, options, type, onSingle, onMultiple }: {
  form: ChecklistFormState; label: string; name: string; options: ChecklistOption[]; type: "radio" | "checkbox";
  onSingle: (name: string, value: string) => void; onMultiple: (name: string, value: string, checked: boolean) => void;
}) {
  ...
  return (
    <Field label={label}><div className={styles.options}>
...
export function StepCard(props: StepCardProps) {
  const { currentStep, form, minDate, setText, setSingle, setMultiple, setFiles, setConsent, setError } = props;
  const choices = (label: string, name: string, items: ChecklistOption[], type: "radio" | "checkbox" = "radio") => <ChoiceField form={form} label={label} name={name} onMultiple={setMultiple} onSingle={setSingle} options={items} type={type} />;
    ...
    <section className={styles.stepCard}>
      ...
      <div className={styles.fields}>
        {currentStep === 0 && <>
          ...
          {form.renovationReason === "기타" && <TextField form={form} label="기타 사유" name="renovationReasonOther" onChange={setText} placeholder="인테리어를 고려하게 된 이유를 입력해주세요." />}
        </>}
        ...
        {currentStep === 2 && <>
          ...
          {visibleSpaces.length > 0 && <Field hint="선택한 공간에서 바꾸고 싶은 항목을 골라주세요." label="공간별로 바꾸고 싶은 항목"><div className={styles.spaceGroups}>
            ...
            {visibleSpaces.includes("기타") && <div><strong>기타 공간</strong><textarea name="spaceDetailsOther" onChange={(event) => setText("spaceDetailsOther", event.target.value)} placeholder="공간과 바꾸고 싶은 항목을 입력해주세요." value={form.spaceDetailsOther} /></div>}
          </div></Field>}
          <TextField form={form} label="현재 공간에서 가장 불편하거나 개선하고 싶은 부분" name="inconvenience" onChange={setText} placeholder="예: 수납이 부족해요 / 주방이 좁고 불편해요 / 공간이 전체적으로 어두워 보여요" textarea />
          <TextField form={form} label="하지 않아도 되는 공사" name="skipOk" onChange={setText} placeholder="예: 베란다는 꼭 안 해도 됩니다. 창호는 상태를 보고 결정하고 싶어요." textarea />
          {choices("가장 중요하게 생각하는 기준", "priority", options.priority, "checkbox")}
          <TextField form={form} label="절대 포기하기 어려운 부분" name="nonNegotiable" onChange={setText} placeholder="예: 욕실은 꼭 새것처럼 바꾸고 싶어요. 수납은 최대한 확보하고 싶어요." textarea />
        </>}
        ...
        {currentStep === 4 && <>
          ...
          {form.styles.includes("기타") && <TextField form={form} label="원하는 스타일 기타" name="otherStyle" onChange={setText} placeholder="원하는 스타일이나 분위기를 자유롭게 적어주세요." textarea />}
          {choices("선호 색감", "colorTone", options.colorTone, "checkbox")}
          <TextField form={form} label="피하고 싶은 느낌" name="avoidStyle" onChange={setText} placeholder="예: 너무 차가운 느낌은 싫어요. 어두운 집은 피하고 싶어요." textarea />
        </>}
        ...
        {currentStep === 5 && <>
          ...
          <TextField form={form} label="생활 방식 및 취미 관련 특이사항" name="lifestyleNote" onChange={setText} placeholder="예: 홈짐이 필요해요 / 악기 연주를 해요 / 캠핑 장비가 많아요 / 큰 작품을 만드는 취미가 있어요" textarea />
        </>}
        ...
        {currentStep === 6 && <>
          ...
          <TextField form={form} label="참고 링크" name="referenceLinks" onChange={setText} placeholder="인스타그램 / 블로그 / 핀터레스트 / 타업체 시공사례 / 제품 링크 등을 붙여넣어주세요." textarea />
          <TextField form={form} label="참고 자료에서 마음에 드는 부분" name="referenceLike" onChange={setText} placeholder="예: 전체 분위기 / 색감 / 주방 디자인 / 욕실 분위기 / 조명 / 수납 방식 / 바닥재 느낌 / 가구 배치" textarea />
        </>}
        ...
        {currentStep === 7 && <>
          ...
          <TextField form={form} label="상담 때 꼭 물어보고 싶은 내용" name="questions" onChange={setText} placeholder="예: 이 예산으로 어디까지 가능한지 궁금해요. 공사 기간이 얼마나 걸릴지 궁금해요." textarea />
          <TextField form={form} label="기타 요청사항" name="etc" onChange={setText} placeholder="추가로 전달하고 싶은 내용을 적어주세요." textarea />
          <TextField form={form} label="성함" name="name" onChange={setText} placeholder="성함을 입력해주세요." />
          <TextField form={form} inputMode="numeric" label="휴대폰 번호" name="phone" onChange={setText} type="tel" />
          <div className={styles.consentBox}><label><input checked={form.privacyConsent} name="privacyConsent" onChange={(event) => setConsent(event.target.checked)} type="checkbox" /><span><strong>개인정보 수집 및 이용에 동의합니다.</strong><small>상담 접수와 연락을 위해 성명, 연락처, 주소 및 상담 내용을 수집하며 상담 완료 후 관련 기준에 따라 보관·파기합니다.</small></span></label></div>
