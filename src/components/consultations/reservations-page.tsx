...
export function ReservationsPage({initialScheduled,received,todayKey}:{initialScheduled:Consultation[];received:Consultation[];todayKey:string}) {
  ...
  const section=(title:string,items:Consultation[],empty:string)=><section><h2>{title} <strong>{items.length}건</strong></h2>{items.length?items.map((item)=><ReservationCard key={item.id} item={item} onEdit={()=>setTarget(item)}/>):<div className="reservation-empty">{empty}</div>}</section>;
